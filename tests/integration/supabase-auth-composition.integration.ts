...
describeIntegration('Phase 2-C5-B real Supabase Auth composition', { concurrent: false }, () => {
  ...
  beforeAll(async () => {
    ...
    const activeStudent = await fixtures.createIdentity('c5b-active-student', 'student', 'active');
    const activeTeacher = await fixtures.createIdentity('c5b-active-teacher', 'teacher', 'active');
  ...
  it('reports a missing Profile, blocks content, and recovers through explicit refresh', async () => {
    ...
    try {
      ...
      const authState = await harness.waitForAuthState(
        (state) =>
          state.status === 'authenticated' && state.user.id === missingProfileStudent.user.id,
