...
describe('authorizeOperation', () => {
  ...
  it('يسمح للمعلم النشط بعملية التأليف', () => {
    expect(decide('author_content', authorized('teacher'))).toEqual({
      allowed: true,
      reason: 'allowed',
    });
  });
  ...
  it('لا يسمح للمراجع suspended بعملية review_content', () => {
    expect(
      decide('review_content', {
        status: 'suspended',
